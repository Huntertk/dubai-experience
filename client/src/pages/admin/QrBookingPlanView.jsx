...
const QrBookingPlanView = () => {
  ...
  return (
    <div className='qr-code-view-main-container'>
        ...
        <p>Total Result : {qrData?.qrCodes?.length}</p>
        <p>Total Adult : {qrData?.qrCodes?.filter((val) => val.Type === 'Adult').length}</p>
        <p>Total Child : {qrData?.qrCodes?.filter((val) => val.Type === 'Child').length}</p>
